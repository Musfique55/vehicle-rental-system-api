...
const createBooking = async (payload: Record<string, unknown>) => {
  ...
  const checkDate = new Date(rent_start_date as string) < new Date(rent_end_date as string);
...
const updateBookingCustomer = async (id: string, status: string) => {
  ...
  const isRentStarted = new Date (booking.rent_start_date) > new Date();

  if(isRentStarted){
    throw new Error("sorry booking can be cancelled only before start date only");
  }
...
const updateBookingAdmin = async (id: string, status: string) => {
  ...
  if(result.rows.length){
      ...
      await pool.query(`UPDATE vehicles SET availability_status=$1 WHERE id=$2 RETURNING availability_status`, [
        "available",
        thisVehicle.vehicle_id,
      ]);
      const vehicleStatus = (await vehicleServices.getVehicle(thisVehicle.vehicle_id)).rows[0].availability_status;
      const vehicle = {
